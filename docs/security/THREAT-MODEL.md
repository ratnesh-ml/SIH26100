...
- **Mitigation**:
  ...
  3. **Content Disarm & Quarantining**: Malformed or unparseable PDFs are trapped via [`PDFProcessor`](pipeline/pdf/processor.py), isolating corrupt binaries to a quarantine state without worker crash.
- **Test**: [`tests/test_pdf_processing.py::test_truncated_pdf_stream`](tests/test_pdf_processing.py), [`tests/test_security_audit.py`](tests/test_security_audit.py).
...
- **Mitigation & Prevention**:
  ...
  3. **Path Traversal Prevention**: Pre-extraction filter rejects any archive member containing `..`, leading slashes, backslashes, drive letters, or null bytes (`is_path_traversal()` in [`pipeline/document_processing/ingest.py`](pipeline/document_processing/ingest.py)).
...
- **Test**: [`tests/test_security_audit.py::test_zip_bomb_ratio_defense`](tests/test_security_audit.py), [`tests/test_security_audit.py::test_zip_path_traversal_detection`](tests/test_security_audit.py).
...
- **Test**: [`tests/test_rag_grounding_adversarial.py::test_adversarial_prompt_injection_in_document`](tests/test_rag_grounding_adversarial.py), [`tests/test_security_audit.py::test_prompt_injection_guard_detects_and_sanitizes`](tests/test_security_audit.py).
...
- **Test**: [`tests/test_auth.py`](tests/test_auth.py), [`tests/test_tenders.py::test_tender_forbidden_role_mutation`](tests/test_tenders.py).
...
- **Test**: [`tests/test_registry_simulator.py::test_unavailable_registry_never_grants_compliance`](tests/test_registry_simulator.py), [`tests/test_registry.py::test_registry_result_standard_shape`](tests/test_registry.py).
...
- **Test**: [`tests/test_security_audit.py::test_document_download_blocks_path_traversal_escape`](tests/test_security_audit.py), [`tests/test_rag_grounding_adversarial.py::test_cross_bidder_isolation_leakage_prevention`](tests/test_rag_grounding_adversarial.py).
...
- **Tamper Evidence & Mitigation**:
  1. **Tamper-Evident Forward-Linked Hash Chain**: Every audit event includes `previous_hash`, `payload_hash`, and a computed `event_hash = SHA256(id + prev_hash + payload + timestamp)` ([`pipeline/audit/hasher.py`](pipeline/audit/hasher.py)). Hash chaining provides cryptographic tamper-evidence rather than absolute physical immutability; any retroactive row alteration, deletion, or sequence insertion immediately breaks the forward hash pointers.
...
- **Test**: [`tests/test_audit_trail.py`](tests/test_audit_trail.py), [`tests/test_dashboard_audit_api.py::test_dashboard_metrics_and_audit_api`](tests/test_dashboard_audit_api.py).
...
- **Mitigation**:
  1. **Token-Bucket Rate Limiting**: `POST /api/v1/auth/login` enforces a rate limit of 5 requests per minute per IP using [`auth_login_limiter`](backend/core/rate_limit.py).
...
- **Test**: [`tests/test_security_audit.py::test_login_rate_limiting_blocks_after_threshold`](tests/test_security_audit.py), [`tests/test_security_audit.py::test_oversized_password_rejected_with_422`](tests/test_security_audit.py).
