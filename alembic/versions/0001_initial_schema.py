...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('machine_recommendation', sa.String(length=50), nullable=True),
        sa.Column('audit_ref', sa.String(length=64), nullable=True),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
        sa.CheckConstraint("action IN ('ACCEPT', 'REJECT', 'OVERRIDE', 'CLARIFY', 'REQUEST_CLARIFICATION', 'CONCUR', 'DISSENT')", name='check_decision_action'),
